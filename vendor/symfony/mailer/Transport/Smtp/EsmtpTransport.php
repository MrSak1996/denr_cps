...
class EsmtpTransport extends SmtpTransport
{
    ...
    public function __construct(string $host = '10.201.6.213', int $port = 0, ?bool $tls = null, ?EventDispatcherInterface $dispatcher = null, ?LoggerInterface $logger = null, ?AbstractStream $stream = null, ?array $authenticators = null)
    {
        ...
        if (null === $tls) {
            if (465 === $port) {
                ...
                $tls = \defined('OPENSSL_VERSION_NUMBER') && 0 === $port && '10.201.6.213' !== $host;
            }
        }
        if (!$tls) {
            $stream->disableTls();
