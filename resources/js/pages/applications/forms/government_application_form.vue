...
const StepGovernmentApplicant = defineAsyncComponent(() =>
    import('./components/steps/StepGovernmentApplicant.vue')
)
...
const updateStepInUrl = () => {
    const routeName = isEdit.value
        ? 'applications.edit'
        : 'applications.create.government'
    router.visit(
        route(routeName, {
...
const nextStep = async (payload: any) => {
    ...
    try {
        ...
        if (isCreate.value) {
            router.visit(
                route('applications.create.government', {
                    application_id: form.value.application_id,
                    type: props.type,
                    step: currentStep.value,
                }),
...
const handleAcceptPrivacy = async () => {
    const data = await accept(userId,"Government")
