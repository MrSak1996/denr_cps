...
const fetchCities = async (provinceCode: number | string, targetField: string) => {
    ...
    try {
        const { data } = await axios.get(`http://cps.denrcalabarzon.com/api/provinces/${provinceCode}/cities`);
...
const fetchBarangays = async () => {
    ...
    try {
        const { data } = await axios.get('http://cps.denrcalabarzon.com/api/barangays', {
...
const saveCompanyApplication = async () => {
    ...
    try {
        const response = await insertFormData('http://cps.denrcalabarzon.com/api/chainsaw/company_apply', {
            ...company_form,
            ...formData,
            encoded_by: userId,
        });
