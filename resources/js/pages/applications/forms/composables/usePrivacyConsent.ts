...
export function usePrivacyConsent() {
    ...
    const generateApplicationNumber = async (userId: any,application_type:any) => {
    const res = await axios.get('/generateApplicationNumber', {
        params: { user_id: userId,application_type:application_type }
    });
    ...
    const accept = async (userId: any,application_type: any) => {
        hasAgreedPrivacy.value = true
        showPrivacyDialog.value = false
        sessionStorage.setItem('privacy_accepted', 'true')

        return await generateApplicationNumber(userId,application_type)
    }
