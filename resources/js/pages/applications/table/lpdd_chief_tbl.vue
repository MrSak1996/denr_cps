...
const applicantsTable = async () => {
    try {
        ...
        const { applications: endorsedApplications, count: endorsedCount } = await ProductService.getApplicationsByStatus(STATUS_ENDORSED_LPDD_CHIEF, officeId);
        ...
        onConfirm: async (data?: { remarks?: string; returnTo?: string | number }) => {
            try {
                ...
                toast.add({
                   ...
                   setTimeout(() => {
                    router.visit('/dashboard/lpdd-chief');
                }, 1000);
            } catch (error) {
                toast.add({
                    severity: 'error',
...
const buttonState = (row: any) => {
    const isReceived = row.application_status === STATUS_RECEIVED_LPDD_CHIEF;
    const isEndorsed =
