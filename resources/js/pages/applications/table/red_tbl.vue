...
const getSignatories = async (id) => {
    ...
    try {
        const response = await axios.get(`http://cps.denrcalabarzon.com/api/getSignatories/${id}`);
...
const getApplicantFile = async (id) => {
    try {
        const response = await axios.get(`http://cps.denrcalabarzon.com/api/getApplicantFile/${id}`);
...
const getApplicationDetails = async (id) => {
    isloadingSpinner.value = true;
    try {
        const response = await axios.get(`http://cps.denrcalabarzon.com/api/getApplicationDetails/${id}`);
...
const saveApplicantDetails = async () => {
    try {
        ...
        const response = await axios.put(`http://cps.denrcalabarzon.com/api/updateApplicantDetails/${applicationDetails.value.id}`, editableApplicant);
...
const saveChainsawDetails = async () => {
    try {
        ...
        const response = await axios.put(`http://cps.denrcalabarzon.com/api/updateChainsawInformation/${applicationDetails.value.id}`, editableChainsaw);
...
const handleEndorseApplicationStatus = async () => {
    try {
        ...
        const response = await axios.put(`http://cps.denrcalabarzon.com/api/updateApplicationStatus/${applicationDetails.value.id}`, {
            ...
            <div v-for="app in endorsed_applications" :key="app.id" class="border rounded-lg p-3 shadow-sm bg-white">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div>
                        ...
                        <Tag value="Approved" severity="success" class="text-xs" v-if="app.status_title == 'Approved by Regional Executive Director'" />
                        <Tag value="Endorsed to RED" severity="success" class="text-xs" v-if="app.status_title == 'Endorsed to Regional Executive Director'" />
                    </div>
                    ...
                    <div class="flex gap-2">
                        ...
                        <!-- APPROVE -->
                        <Button style="background-color: #0f766e"
                            v-if="app.application_status === STATUS_APPROVED_BY_RED" @click="generatePdf(app)"
                            class="p-2 text-white bg-blue-900">
                            <PrinterCheck :size="14" />
                        </Button>

                        <!-- VIEW -->
                        <Button style="background-color: #0f766e" class="p-2 text-white">
                            <Link :href="route('applications.edit', {
                                id: app.id,
